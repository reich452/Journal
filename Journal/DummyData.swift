...
extension String {
	var words: [String] {
		return self.componentsSeparatedByCharactersInSet(.whitespaceAndNewlineCharacterSet())
	}
...
extension NSDate {
	class func randomDateBefore(date: NSDate) -> NSDate {
		let limit = date.timeIntervalSinceReferenceDate
		let randomTime = NSTimeInterval(arc4random_uniform(UInt32(limit)))
		return NSDate(timeIntervalSinceReferenceDate: randomTime)
	}
}
...
func addDummyEntries(numberOfEntries: Int) {
	...
	for _ in 0..<numberOfEntries {
		...
		let timestamp = NSDate.randomDateBefore(now)
